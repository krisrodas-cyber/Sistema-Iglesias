...
const appendRow = (priest) => {
  ...
  [priest.codigo, fullName(priest), priest.tipo_clero, priest.congregacion || '—', priest.telefono || '—'].forEach((value, index) => { const cell = document.createElement('td'); cell.textContent = value; if (index === 1) cell.className = 'priest-table__name'; row.append(cell); });
...
export const destroySacerdotes = () => { eventController?.abort(); eventController = null; formModal?.dispose(); detailModal?.dispose(); statusModal?.dispose(); toast?.dispose(); formModal = null; detailModal = null; statusModal = null; toast = null; document.querySelectorAll('[data-view-overlay="sacerdotes"]').forEach((overlay) => overlay.remove()); };
