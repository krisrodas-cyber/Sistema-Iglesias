...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'create') { if (!/^\S+@\S+\.\S+$/.test(body.email ?? '') || String(body.password ?? '').length < 10 || !body.nombre?.trim() || !body.apellido?.trim() || !validRole(body.rol_id)) return fail(origin, 'INVALID_INPUT', 'Datos de usuario inválidos.', 400); const parishResult = await resolveParishForRole(body.rol_id, body.parroquia_id); if (parishResult.error) return fail(origin, parishResult.error.code, parishResult.error.message, parishResult.error.status); const { data, error: createError } = await adminClient.auth.admin.createUser({ email: body.email.trim(), password: body.password, email_confirm: true, ban_duration: body.activo === false ? '876000h' : 'none' }); if (createError || !data.user) return fail(origin, 'CONFLICT', 'No fue posible crear el usuario.', 409); const { error: profileInsertError } = await adminClient.from('perfiles').insert({ id: data.user.id, nombre: body.nombre.trim(), apellido: body.apellido.trim(), rol_id: String(body.rol_id), parroquia_id: parishResult.parishId, activo: body.activo !== false }); if (profileInsertError) { await adminClient.auth.admin.deleteUser(data.user.id); return fail(origin, 'PROFILE_CREATION_FAILED', 'El usuario fue creado, pero no fue posible completar su perfil.', 500); } return respond(origin, { ok: true, data: { id: data.user.id, email: data.user.email, nombre_completo: `${body.nombre.trim()} ${body.apellido.trim()}`, parroquia_id: parishResult.parishId } }, 201); }
    ...
    if (action === 'changeRole') { if (!validRole(body.rol_id)) return fail(origin, 'INVALID_ROLE', 'Rol inválido.', 400); if (targetId === user.id) return fail(origin, 'CONFLICT', 'No puede modificar su propio rol.', 409); if (String(body.rol_id) !== '1' && await lastAdmin(targetId)) return fail(origin, 'CONFLICT', 'No puede degradar al último Administrador activo.', 409); const parishResult = await resolveParishForRole(body.rol_id, body.parroquia_id); if (parishResult.error) return fail(origin, parishResult.error.code, parishResult.error.message, parishResult.error.status); const { error } = await adminClient.from('perfiles').update({ rol_id: String(body.rol_id), parroquia_id: parishResult.parishId }).eq('id', targetId); return error ? fail(origin, 'INTERNAL_ERROR', 'No fue posible cambiar el rol.', 500) : respond(origin, { ok: true, data: { id: targetId, parroquia_id: parishResult.parishId } }); }
